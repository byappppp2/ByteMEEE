...
def main():
    ...
    weights = {
        ...
        "flag_cross_bank": 2,
        "flag_same_account": 3,
        "flag_odd_hours": 1,                 # soft
        "flag_cross_currency": 1,            # soft
        "flag_round_amount": 1,              # soft
        "flag_structuring": 3,
        "flag_velocity_outgoing": 2,
        "flag_counterparty_diversity": 2
    }

    present = [c for c in rule_cols if c in df.columns]
    for c in present:
        df[c] = df[c].fillna(False).astype(bool)

    df["final_score"] = np.zeros(len(df), dtype=np.int16)
    for c in present:
        df["final_score"] += df[c].astype(np.int16) * np.int16(weights[c])

    df["severity"] = pd.cut(
        df["final_score"],
        bins=[-1, 2, 5, 8, 99],   # Low ≤2, Medium 3–5, Elevated 6–8, High ≥9
